...
- `An ideal production configuration.md` — the **P3 (production readiness) plan**, re-planned 2026-09-26 as **version 2.0**. Target production architecture (now **single-host**, not the old 3-VM split — see its ADR-005), network policy, security model, DR plan, ADRs, and an implementation plan structured as **evidence-gated tiers P3.0–P3.7** rather than a 90-day calendar. Still a design/roadmap document — P3 remains 0% implemented and the file carries a status banner saying so — but it is now accurate about the current tree: its §11 lists the sixteen-plus corrections made to the 2026-07-30 draft, several of which were actively harmful (it named the wrong Alfresco image, proposed a pre-built AtroCore image whose GPL-3.0 problem was deliberately engineered away, and omitted four required containers from its sizing). Two things in it are load-bearing for anyone touching deployment: the three shared Docker networks are **single-host bridge networks**, which is why multi-host is deferred; and `compliance_cmis`'s Traefik and `compliance_web`'s `prod` profile **both bind host port 8080**, so the production profile cannot currently start next to the Alfresco stack.
