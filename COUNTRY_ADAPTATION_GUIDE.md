...
**To adapt**: replace `entityName` with your authority's name in each locale you ship, drop your logo over `configs/entity-logo.png` (or point `entityLogoPath` at your own file), and fill in the document-control codes/version/date if you use them. The plan/report **signature blocks** are configured in the same file's `signatures` block (`planApprovedBy`/`planApprovedByPosition`, `reportApprovedBy`/`reportApprovedByPosition`, and `signatureDate`), all blank by default; a caller can also name an approver per inspection through the endpoints' `approvedBy`/`approvedByPosition`/`signatureDate` fields, and the plan flow defaults `approvedBy` to the planner who defined the site visit. The per-report titles and subtitles live separately, in the locale label dictionaries in the webscripts (`INFORME_FINAL_LABELS`, `PLAN_LABELS`, `REPORT_LABELS`) — edit `docTitle`/`docSubtitle` there to change wording. Restart the Alfresco container to pick up the change (bind-mounted, but Alfresco doesn't watch the file for live reload). After editing any `templates/*.fodt`, re-run `compliance_cmis/scripts/bootstrap-site-content.sh --yes --force` so the instance replaces its already-filed copies.
